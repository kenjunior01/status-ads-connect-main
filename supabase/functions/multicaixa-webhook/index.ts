...
serve(async (req) => {
  ...
  try {
    ...
    const rlRes = await fetch(`${supabaseUrl}/rest/v1/function_logs?provider=eq.multicaixa&created_at=gte.${windowStart}&message=eq.${encodeURIComponent(ip)}`, {
    ...
    if ((rl || []).length >= 10) {
      await fetch(`${supabaseUrl}/rest/v1/function_logs`, {
        ...
        body: JSON.stringify({ provider: "multicaixa", level: "error", message: `rate_limited ${ip}` }),
      });
    ...
    await fetch(`${supabaseUrl}/rest/v1/function_logs`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "apikey": supabaseKey, "Authorization": `Bearer ${supabaseKey}`, "Prefer": "return=representation" },
      body: JSON.stringify({ provider: "multicaixa", level: "info", message: ip }),
    });
    const raw = await req.text();
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const supabaseKey = Deno.env.get("SUPABASE_ANON_KEY")!;
    const secret = Deno.env.get("MULTICAIXA_WEBHOOK_SECRET") ?? "";
    ...
    if (!event_id || !existing?.length) {
      await fetch(`${supabaseUrl}/rest/v1/payment_events`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "apikey": supabaseKey, "Authorization": `Bearer ${supabaseKey}`, "Prefer": "return=representation" },
        body: JSON.stringify({ provider: "multicaixa", user_id, campaign_id, subscription_id, status: status || "confirmed", amount, event_id: idToUse || null, payload: body }),
      });
    }
    ...
    try {
      ...
      await fetch(`${supabaseUrl}/rest/v1/function_logs`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "apikey": supabaseKey, "Authorization": `Bearer ${supabaseKey}`, "Prefer": "return=representation" },
        body: JSON.stringify({ provider: "multicaixa", level: "error", message: e instanceof Error ? e.message : String(e) }),
      });
    } catch { /* noop */ }
