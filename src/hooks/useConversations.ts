...
export const useMessages = (conversationId: string | null) => {
  ...
  const { toast } = useToast();
  const queueKey = conversationId ? `offline_messages_${conversationId}` : null;
  ...
  const sendMessage = async (content: string, attachment?: { url: string; type: string; name: string }) => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        ...
        const existing: Message[] = queueKey ? JSON.parse(localStorage.getItem(queueKey) || '[]') : [];
        existing.push(pending);
        if (queueKey) localStorage.setItem(queueKey, JSON.stringify(existing));
        setMessages(prev => [...prev, pending]);
        toast({ title: "Offline", description: "Mensagem enfileirada e será enviada ao reconectar." });
        return pending;
      }
  ...
  const flushOfflineQueue = useCallback(async () => {
    if (!conversationId || !navigator.onLine || !queueKey) return;
    try {
      const pending = JSON.parse(localStorage.getItem(queueKey) || '[]');
      if (!Array.isArray(pending) || pending.length === 0) return;
      for (const item of pending) {
        await sendMessage(item.content, item.attachment_url ? { url: item.attachment_url, type: item.attachment_type, name: item.attachment_name } : undefined);
      }
      localStorage.removeItem(queueKey);
      toast({ title: "Reconectado", description: "Mensagens offline enviadas." });
    } catch (e) {
      console.error("Flush offline queue error:", e);
    }
